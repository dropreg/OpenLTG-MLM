    ...
    with open(source_file, 'w') as source_w, open(target_file, "w") as target_w:
        for raw_lines in open(raw_file).readlines():
            ...
            if len(target_sentence) > 1:            
                bound = len(target_sentence) // 4
                
                for bound_idx in range(4):
                    bound_start = bound * bound_idx
                    bound_end = bound_start + bound
                    if bound_idx == 3 and bound_end < len(target_sentence) - 1:
...
def split_writingprompt():
    ...
    root_dir = "/opt/data/private/data/arr1015_data/writing_prompt/seg_four/"
    # Train File:
