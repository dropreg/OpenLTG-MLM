    ...
    with open(source_file, 'w') as source_w, open(target_file, "w") as target_w:
        for raw_lines in open(raw_file).readlines():
            ...
            if len(target_sentence) > 1:            
                ...
                if bound > 1:
                    target_w.write(clip(" ".join(target_sentence[:bound])) + "\n")
                else:
                ...
                if bound < len(target_sentence) - 1:
                    ...
                    target_w.write(clip(target_sentence[len(target_sentence) - 1]) + "\n")
            else:
                target_w.write(clip(target_sentence[0]) + "\n")
...
def split_writingprompt():
    ...
    root_dir = "/opt/data/private/data/arr1015_data/writing_prompt/seg_two/"
    # Train File:
